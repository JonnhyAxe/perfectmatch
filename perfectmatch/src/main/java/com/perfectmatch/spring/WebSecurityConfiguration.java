...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(HttpSecurity http) throws Exception {
    ...
    // .disable().authorizeRequests().anyRequest().fullyAuthenticated().and()
    // .oauth2ResourceServer().jwt();

    http.authorizeRequests().anyRequest().fullyAuthenticated().and().oauth2Client().and()
      ...
      authorities.forEach(authority -> {
        if (authority instanceof OidcUserAuthority) {
          ...
          List<SimpleGrantedAuthority> groupAuthorities = userInfo.getClaimAsStringList("groups")
              .stream().map(g -> new SimpleGrantedAuthority("ROLE_" + g.toUpperCase()))
