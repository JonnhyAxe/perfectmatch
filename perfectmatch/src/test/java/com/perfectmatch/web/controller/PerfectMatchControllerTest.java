...
public class PerfectMatchControllerTest {
	 ...
	 private static final String CHECK_THAT_PERFECT_MATCH_IS_RETREIVED = "Check that perfect match is retrieved";
	 
	 private static final String CHECK_THAT_PERFECT_MATCH_FIELDS_ARE_FILLED_IN = "Check that erfect match fields are filled in";
	 
	 private static final String CHECK_THAT_LIST_CONTENT_IS_CORRECT = "Check that list content is correct";

	 private static final String CHECK_THAT_PERFECT_MATCHS_ARE_RETREIVED = "Check that perfect matchs are retreived";

	 private static final String CHECK_THAT_PERFECT_MATCH_DOES_NOT_EXIST = "Check that perfect match does not exist";
	 
	 private static final String CHECK_THAT_ERROR_MESSAGE_IS_CORRECT = "Check that error message is correct";
	 
	 private MockMvc mvc;
		  
	 @InjectMocks private PerfectMatchController perfectMatchController;

	 @Mock private PerfectMatchServiceBean perfectMatchServiceBean;
	  ...
	  private JacksonTester<List<PerfectMatch>> 	jsonPerfectMatchs;
	  ...
	  public void getAllPerfefctMatchs() throws Exception {
		  List<PerfectMatch> expectedPerfectMatchs = Arrays.asList(new PerfectMatch());
		 
	      given(perfectMatchServiceBean.findAll())
	        .willReturn(expectedPerfectMatchs);
	      ...
	      then(response.getStatus())
	      	.as(CHECK_THAT_PERFECT_MATCHS_ARE_RETREIVED)
	      	.isNotNull()
	  ...
	  public void canRetrieveByNameWhenExists() throws Exception {
	      ...
	      given(perfectMatchServiceBean.findPerfectMatchByName(matchName))
	        .willReturn(expectedPerfectMatch);

	      // when
	      MockHttpServletResponse response = mvc.perform(
	              get("/perfect_match/"+ matchName)
	              	.accept(MediaType.APPLICATION_JSON)
	              )
	              .andReturn().getResponse();

	      then(response.getStatus())
	      	.as(CHECK_THAT_PERFECT_MATCH_IS_RETREIVED)
	      	.isNotNull()
	      	.isEqualTo(HttpStatus.OK.value());
	      then(response.getContentAsString())
	      	.as(CHECK_THAT_PERFECT_MATCH_FIELDS_ARE_FILLED_IN)
	  ...
	  public void canRetrieveByPerfectMatchNameWhenDoesNotExists() throws Exception {
	      ...
	      then(response.getStatus())
	      	.as(CHECK_THAT_PERFECT_MATCH_DOES_NOT_EXIST)
	      	.isNotNull()
