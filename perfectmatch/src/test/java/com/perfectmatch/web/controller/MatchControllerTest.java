...
public class MatchControllerTest {
	 ...
	 @Mock private SampleMatchRepository sampleMatchRepository;
	  ...
	  public void setup() {
	      ...
	      mvc = MockMvcBuilders.standaloneSetup(matchController)
	              .setControllerAdvice(new RestResponseEntityExceptionHandler(),
	            		  new MatchControllerExceptionHandler()
	            		  )
	              .build();
	  }
	  ...
	  public void getAllMatchs() throws Exception {
	      ...
	      then(response.getStatus())
	      	.as(CHECK_THAT_MATCHS_ARE_RETREIVED)
	      	.isNotNull()
	  ...
	  public void canRetrieveByNameWhenExists() throws Exception {
	      ...
	      // when
	      MockHttpServletResponse response = mvc.perform(
	              get("/match/"+ AWESOME_MUSIC_NAME_THIS + "/" + AWESOME_MUSIC_NAME_THAT)
	              	.accept(MediaType.APPLICATION_JSON)
	              )
	              .andReturn().getResponse();

	      then(response.getStatus())
	      	.as(CHECK_THAT_MATCH_IS_RETREIVED)
	      	.isNotNull()
	      	.isEqualTo(HttpStatus.OK.value());
	      then(response.getContentAsString())
	      	.as(CHECK_THAT_MATCH_FIELDS_ARE_FILLED_IN)
	  ...
	  public void canRetrieveByNameWhenExistsNamesInversed() throws Exception {
	      ...
	      // when
	      MockHttpServletResponse response = mvc.perform(
	              get("/match/"+ AWESOME_MUSIC_NAME_THAT + "/" + AWESOME_MUSIC_NAME_THIS)
	              	.accept(MediaType.APPLICATION_JSON)
	              )
	              .andReturn().getResponse();
	  ...
	  public void canRetrieveByNameWhenDoesNotExists() throws Exception {
	      ...
	      // when
	      MockHttpServletResponse response = mvc.perform(
	              get("/match/fakeMusicName1/fakeMusicName2")
	              	.accept(MediaType.APPLICATION_JSON)
	              )
	              .andReturn().getResponse();

	      then(response.getStatus())
	      	.as(CHECK_THAT_MATCH_DOES_NOT_EXIST)
	      	.isNotNull()
	  ...
	  public void findAllMatchByMusic() throws Exception {
		  ...
		  List<Match> expectedListMatchs =  Arrays.asList(expectedMatch, expectedMatch2);
	      ...
	      // when
	      MockHttpServletResponse response = mvc.perform(
	              get("/match/" + AWESOME_MUSIC_NAME_THIS)
	              	.accept(MediaType.APPLICATION_JSON)
	              )
	              .andReturn().getResponse();
	  ...
	  public void createMatch() throws Exception {
	      ...
	      given(sampleMatchRepository.save(expectedMatch))
	       .willReturn(expectedMatch);
	      ...
	      MockHttpServletResponse response = mvc.perform(
	              post("/match")
	              	.contentType(MediaType.APPLICATION_JSON)
	              	.content(jsonMatch.write(expectedMatch).getJson()
	                )
	              ).andReturn().getResponse();
